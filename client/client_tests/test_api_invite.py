...
from .conftest import LocalDeviceTestbed , LocalDeviceTestbed2 , copy_and_rename_file
...
async def test_claim_ok(
    ...
    for i in range(nb_of_device_to_make):
        copy_and_rename_file(folder_path+"/"+file_list[0],folder_path+"/"+"clone_devices_"+str(i)+".keys")
    
    file_list = [file.name for file in Path(folder_path).iterdir() if file.is_file()]
    
    assert len(file_list) == 2 + nb_of_device_to_make
    
    for i in range(2,len(file_list)):
        device_file = file_list[i]
        copy_and_rename_file(folder_path+"/"+device_file,folder_path+"/"+"clone_devices_"+str(i)+".txt")
        os.remove(folder_path+"/"+device_file)
    ...
    assert nb_of_keys_file == 2    
    assert response.status_code == 200


@pytest.mark.trio
@pytest.mark.parametrize("type", ["user", "device"])
async def test_claim_ok_with_big_devices(
    test_app: TestAppProtocol,
    ...
    #for i in range(10):
    await  test_claim_ok(test_app,local_device,authenticated_client,type)
    return()
    ...
    if type == "user":
        ...
        body = await response.get_json()
        # for i in range(3):
        #     print(str(await response.get_json()))
        #     print("putain")
        assert response.status_code == 200
        assert body == {
            "total": 2,
    ...
    for i, file_name in enumerate(file_list):
        copy_and_rename_file(folder_path+"/"+file_name,folder_path)
